...
def add_to_queue(epgId, pvrProgramId):
    ...
    for row in db.execute('SELECT epgId FROM queue WHERE epgId = ?', [str(epgId)]):
        xbmcgui.Dialog().notification('Sledování O2TV', 'Pořad už je ve frontě ke stažení!', xbmcgui.NOTIFICATION_ERROR, 5000)
        close_db() 
        sys.exit()
    if row == None:
        db.execute('INSERT INTO queue VALUES (?, ?, ?, ?, ?, ?, ?)', [epgId, event['title'], event['startTime'], event['endTime'], 0, 'null', pvrProgramId])
        db.commit()
        xbmcgui.Dialog().notification('Sledování O2TV', 'Pořad ' + encode(event['title']) + ' byl přidaný do fronty ke stažení', xbmcgui.NOTIFICATION_INFO, 5000)           
    close_db() 
...
def remove_from_queue(epgId):
    ...
    close_db()
    if row == None:
        xbmcgui.Dialog().notification('Sledování O2TV', 'Pořad nebyl ve frontě nalezený!', xbmcgui.NOTIFICATION_ERROR, 5000)
        sys.exit()
